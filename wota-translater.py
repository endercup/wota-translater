...
def get_ffmpeg_paths():
    # 可能的搜尋路徑：PyInstaller 解壓暫存 / 當前腳本所在資料夾
    search_paths = []
    ...
    for path in search_paths:
        ...
        if os.path.exists(ffmpeg_path) and os.path.exists(ffprobe_path):
            # 設定 DLL 搜尋路徑（避免缺 libmp3lame-0.dll 問題）
            try:
    ...
    try:
        ctypes.windll.user32.MessageBoxW(None, error_msg, "wota-translater", 0x10)  # 0x10 = MB_ICONERROR
    except Exception:
        pass
    raise SystemExit(1)
