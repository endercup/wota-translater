...
# === 初始化 colorama（確保 Windows 終端機也能彩色輸出） ===
...
def get_ffmpeg_paths():
    ...
    search_paths.append(os.path.abspath(os.path.dirname(__file__)))
    ...
    for path in search_paths:
        ...
        ffprobe_path = os.path.join(path, ffprobe_name)
    ...
    error_msg = "錯誤：找不到 ffmpeg/ffprobe！請確認它們與程式放在同一資料夾。"
    print(Fore.RED + error_msg + Style.RESET_ALL)
    try:
        ctypes.windll.user32.MessageBoxW(None, error_msg, "wota-translater", 0x10)
    except Exception:
        pass
    raise SystemExit(1)

# === 套用到 pydub ===
FFMPEG_BIN, FFPROBE_BIN = get_ffmpeg_paths()
AudioSegment.converter = FFMPEG_BIN
AudioSegment.ffprobe = FFPROBE_BIN

print(Fore.GREEN + f"✔ 已載入 FFmpeg：{FFMPEG_BIN}" + Style.RESET_ALL)
print(Fore.GREEN + f"✔ 已載入 FFprobe：{FFPROBE_BIN}" + Style.RESET_ALL)
...
def main():
    ...
    intro_lines = [
        f"🎵 wota-translater — 快速剪輯 御宅藝 用副歌檔案",
